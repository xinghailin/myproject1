...
public class CustomExcelHelper {
	...
	public <T> List<T> toEntitys(Class<T> classType, Integer countLine) throws ExcelParseException, ExcelContentInvalidException,
		...
		try {
			...
			int lineNo = 1;
			if (countLine != null) {
				lineNo = countLine.intValue();
			}
			for (int j = countLine.intValue() - 1; j < datas.length; j++) {
				String[] data = datas[j];
				boolean flag = true;
				for (int i = 0; i < eefs.size(); i++) {
					if (StringUtils.isNotBlank(data[i])) {
