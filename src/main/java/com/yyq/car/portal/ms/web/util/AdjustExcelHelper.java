 ...
 */
public class AdjustExcelHelper {
	// 最小列数目
	 ...
	 */
	public static AdjustExcelHelper readExcel(String excelFilename) throws InvalidFormatException, IOException, ExcelContentInvalidException {
	 ...
	 * @throws ExcelContentInvalidException 
	 */
	public static AdjustExcelHelper readExcel(String excelFilename, int sheetIndex) throws InvalidFormatException, IOException, ExcelContentInvalidException {
	 ...
	 * @throws ExcelContentInvalidException 
	 */
	public static AdjustExcelHelper readExcel(File file) throws InvalidFormatException, IOException, ExcelContentInvalidException {
	 ...
	 * @throws ExcelContentInvalidException 
	 */
	public static AdjustExcelHelper readExcel(File file, int sheetIndex) throws InvalidFormatException, IOException, ExcelContentInvalidException {
		// 读取Excel工作薄
	 ...
	 * @throws ExcelContentInvalidException 
	 */
	public static AdjustExcelHelper readExcel(InputStream ins) throws InvalidFormatException, IOException, ExcelContentInvalidException {
	 ...
	 * @throws ExcelContentInvalidException 
	 */
	public static AdjustExcelHelper readExcel(InputStream ins, int sheetIndex) throws InvalidFormatException, IOException, ExcelContentInvalidException {
	...
	public <T> List<T> toEntitys(Class<T> classType) throws ExcelParseException, ExcelContentInvalidException, ExcelRegexpValidFailedException {
		...
		// 处理车型、车架号列
		String[] data2 = datas[1];// 获取第二列
		Map<String, String> map = new HashMap<String, String>();
		for (int i = 6; i < headers.length; i++) {// 从G列第6列开始
			String h = headers[i];
			String d = data2[i];
			if (StringUtils.isNotBlank(h) && StringUtils.isNotBlank(d)) {
				if (h.contains("车型")) {
					if (d.contains("*")) {
						String[] bm = d.split("*");
						if (bm.length == 2) {
							...
							throw new ExcelContentInvalidException("第二行，第" + (i + 1) + "列，车型格式错误，正确格式为：品牌*型号");
						}
					} else {
						throw new ExcelContentInvalidException("第二行，第" + (i + 1) + "列，车型格式错误，正确格式为：品牌*型号");
					}
				} else if (h.contains("车架号") && !d.contains("*")) {
		...
		try {
			...
			for (String[] data : datas) {
				...
				for (ExcelEntityField eef : eefs) {// 仅仅填充有注解的字段
					...
					// if (eef.isRequired() && StringUtils.isBlank(data[eef.getIndex()])) {// 必须
					// throw new ExcelParseException("Excel数据的第" + lineNo + "行第" + (eef.getIndex() +
					// 1) + "列字段" + eef.getColumnName() + "为空!");
					// }
					// todo:alvin:规则校验 end
					// 实体数据填充
	...
	private <T> List<ExcelEntityField> _getEntityFields(Class<T> classType) throws ExcelParseException {
		List<ExcelEntityField> eefs = new ArrayList<AdjustExcelHelper.ExcelEntityField>();
		// 遍历所有字段
	...
	private void _parseExcelHeader(Row row) {
		...
		for (int columnIndex = 0; columnIndex < lastColumnIndex; columnIndex++) {
			...
			String str = _getCellValue(cell).trim();
			if (StringUtils.isBlank(str)) {
				// throw new PortalMsBizUnCheckedException("第一行，第" + (columnIndex + 1) + "列必填");
				break;
			}
			lst.add(str);
			// headers[columnIndex] = str;
		}
		headers = lst.toArray(new String[0]);
		lastColumnIndex = headers.length;
	}
	 ...
	 * @throws ExcelContentInvalidException 
	 */
	private void _parseExcelData(Sheet sheet, int rowStart, int rowEnd) throws ExcelContentInvalidException {
		...
		for (int rowIndex = rowStart; rowIndex <= rowEnd; rowIndex++) {
			...
			if (row != null) {
				...
				for (int columnIndex = 0; columnIndex < lastColumnIndex; columnIndex++) {
					...
					// 如果该行的第一列为空,则结束这个循环
					// if (StringUtils.isBlank(value) && columnIndex == 0) {
					// break;
					// }
					System.err.println("value:----------" + value);
					if (rowNumber == 0 && columnIndex >= 6 && StringUtils.isBlank(value)) {
						throw new ExcelContentInvalidException("第二行，第" + (columnIndex + 1) + "列不能为空");
					}
	 ...
	 * @throws ExcelContentInvalidException 
	 */
	private static AdjustExcelHelper _readExcel(Workbook wb, int sheetIndex) throws ExcelContentInvalidException {
		AdjustExcelHelper eh = new AdjustExcelHelper();
