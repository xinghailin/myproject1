...
//	handlebars.registerHelper('_formatDatetime', _formatDatetime);
	...
	function editClient() {
		...
		if(isview)
		location.href = $('#editUrl').val() + "?process=1&id=" + theId + "&currpage="+ $('#J_CurPageNo').text() + "&view=1";
		else location.href = $('#editUrl').val() + "?process=1&id=" + theId + "&currpage="+ $('#J_CurPageNo').text();
	}
	// 格式化时间
