...
//	handlebars.registerHelper('operate', operate);
	...
	function load(curPageNo, pageSize) {
        dataList.show();
		noDataMsg.hide();
		var queryUrl = $('#searchUrl').val() + "?userCountry="+userCountry+"&start="+curPageNo+"&pageSize="+pageSize+"&forapay=1"; // 查询数据url
		$.ajax(queryUrl, {
			type : 'POST',
			dataType : 'json',
	...
	function havechoose(){
		...
		if(c.length>0){
			...
			window.location.href=$("#subbuyIndexUrl").val()+"?buyid="+theId+"&userCountry="+userCountry+"&currpage="+$('#J_CurPageNo').text();
		}
		else{
