var inx=Math.max($("#max1").val(),$("#max2").val());//图片数组
var picdels=[],picedits=[],dedels=[],deedits=[],dcolor=[],ecolor=[];
var use_personal=$("#use_personal"),uploadurl=$("#uploadUrl").val();//图片上传id数组
function submit(){
use_personal.submit();
if(vil("use_personal")){
//检测配置格式
...
if(bl){return false;}
var up={id:$('#id').val(),displacement:$("#displacement").val(),carprice:$('#carprice').val(),
...
$(".other [type=hidden]:not([value=''])").each(function(){
	//编辑过的
	var th=$(this);
	if(typeof(th.attr("v"))=="undefined"){pics.push(th.val());}
	else{if(th.attr("f")==1){picedits.push(th.attr("v")+";1;"+th.val())}
	else{picedits.push(th.attr("v")+";"+th.val())}}
})
up.pics=pics,up.picedit=picedits,up.picdel=picdels;
if(size){
//	up.version=one.val();//.eq(0).find("option:selected").text();//=='--请选择--'?null:vsion;
...
//	up.configk=kks,up.configv=vrr;
	up.configk=kks.length?JSON.stringify(kks):null;up.configv=vrr.length?JSON.stringify(vrr):null;
}
var des=[];
$('.decoratediv').each(function(){
	var ti=$(this),th=ti.find("input[name^='deconame']");
	var fi=th.val()
	if($.trim(fi).length){//有名称
		if(typeof(th.attr("v"))=="undefined"){
		...
		else{deedits.push({id:th.attr("v"),name:$.trim(fi),con:ti.find("[name^='decodetail']").val(),price:ti.find("[name^='decoprice']").val(),pic:ti.find("[name^='outpic']").val(),ck:ti.find("[name^='default']:checked").val()})}
	}
})
up.decorates=(des.length>0?JSON.stringify(des):null);up.dede=dedels;up.deed=(deedits.length?JSON.stringify(deedits):null);
jQuery.each(jcolors,function(i,ti){
	var th=$(ti);
	var ts=th.find(":text"),f=ts.eq(0),s=ts.eq(1),t=ts.eq(2);
	var fv=$.trim(f.val()),sv=$.trim(s.val()),tv=$.trim(t.val());
	var v=th.find("button");
	if(typeof v.attr("v")=="undefined"){
		jcs.push({outcolor:fv,incolor:sv,price:tv})
	}else{
		ecolor.push({id:v.attr("v"),outcolor:fv,incolor:sv,price:tv})
	}
	})
	up.colors=jcs.length>0?JSON.stringify(jcs):null;up.coloredit=ecolor.length>0?JSON.stringify(ecolor):null;up.colordel=dcolor;
//alert(up)
...
//            console.log(data)
        	...
        	alert(a.responseText);$("#submit").attr("disabled",false);
        }
    })
}
}
...
$('#personal-myCard').delegate('.button-remove','click',function(){
	var th=$(this);
	if(typeof(th.attr("v"))=="undefined"){}
	else{picdels.push(th.attr("v"))}
	th.parent().parent().remove();
})
$('#divcolor').delegate('.btn-danger','click',function(){
	var th=$(this);
	if(typeof(th.attr("v"))=="undefined"){}
	else{dcolor.push(th.attr("v"))}
	th.parent().parent().remove();
})
use_personal
