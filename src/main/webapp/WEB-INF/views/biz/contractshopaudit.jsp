	...
	<div class="main_right" id="viewList" style="margin-left: 300px; height: 100%;">
		<div class="box">
			<div class="container">
				...
				<form name="editForm" id="editForm" method="POST" enctype="multipart/form-data" class="upload" action="<ufa:url namespace="car-ms" fieldName="bizcontractshopaudit"/>"><input type="hidden" id="id" name="id" value="${co.id}">
                ...
                <label>品牌首字母：</label>${in.initials}
					...
					<div class="form-inline">
						<div class="form-group newInput" style="width: 33%;"><label>是否零采：</label> <label class="radio-inline" style="margin-right: 20px"> <input type="radio" name="zeropur" id="inlineRadio1" value="1">是
						</label> <label class="radio-inline"> <input type="radio" name="zeropur" id="inlineRadio2" value="2">否
						</label></div>
						<div class="form-group newInput" style="width: 33%;"><label>状态：</label>${empty co.status ?'门店部待审核':''}</div>
						<div class="form-group newInput" style="width: 33%;"><label>不通过原因：</label><textarea class="form-control" rows="4" style="resize: none; min-width: 300px" id="du-reason1" name="reason"></textarea></div>
				</div></form>
				<div class="du-btn"><c:if test="${empty co.status}">
						<a href="javascript:;" class="btn btn-danger" role="button" style="padding: 6px 30px" id="saveBtn">通过</a>
						<a href="javascript:;" class="btn btn-warning" role="button" style="padding: 6px 30px" id="saveafterupdateBtn">驳回修改</a>
						<a href="javascript:;" class="btn btn-warning" role="button" style="padding: 6px 30px" id="savenotBtn">不通过</a>
...
<script>
seajs.use("${scriptBasePath}/biz/contractshopaudit.js?r=<%=new Date()%>");
</script>
</html>