            ...
            <div class="form-inline">
                ...
                <a href="${contextPath}${co.downposbill}" download="${downposbillname}" style="text-decoration: underline;margin-left: 10px;color: #4d90fe;" title="点击下载">${downposbillname}</a>
					...
					<div class="form-inline">
						...
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%; color: red;"><label>审批金额：</label>${aproveprice}</div>
							<div class="form-group newInput" style="width: 33%; color: red;"><label>缺额：</label>${missmoney}</div>
					...
					<div class="form-inline">
						<div class="form-group newInput" style="width: 33%;"><label>定金：</label>${co.downpay} <c:if test="${co.finance==1 }">
								<label style="color: red;">定金=审批缺额+动态监管费+履约险+合同保险+合同质保</label>
							</c:if></div>
						<div class="form-group newInput" style="width: 33%;"><label>实到定金：</label><input type="text" class="form-control" placeholder="单位：元" name="realdownpay" id="realdownpay"></div>
				</div></form>
				<div class="du-btn"><a href="javascript:;" class="btn btn-danger" role="button" id="saveBtn">确定定金已到账 </a> <a href="javascript:;" class="btn btn-warning" role="button" style="padding: 6px 30px" id="backBtn"> 返回 </a></div>
		</div>
	</div>
</div>
</body>
...
<script>
seajs.use("${scriptBasePath}/biz/contractdownpayaudit.js?r=<%=new Date()%>");
</script>
</html>