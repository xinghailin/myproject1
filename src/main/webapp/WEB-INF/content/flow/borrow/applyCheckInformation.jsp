...
<html lang="en">
  ...
  <div class="main_right" id="queryDiv">
	<div class="rg_list">
	   <div class="tb_list">
		<form id="inputForm" action="${ctx }/loan/checkInformation" method="post" target="mainFrame">
			<input type="hidden" name="processId" value="${processId }" />
			<input type="hidden" name="orderId" value="${orderId }" />
			<input type="hidden" name="taskId" value="${taskId }" />
									
			<input type="hidden" name="S_approveAdmin.operator" value="${nextOperator }" />
			<table class="table_all" align="center" border="0" cellpadding="0"
				cellspacing="0" style="margin-top: 0px">
				<%-- <tr>
					<td class="td_table_1">
						<span>采购价与评估价的匹配结果：</span>
					</td>
					<td class="td_table_2" colspan="3">
						<input type="radio" name="decideResult" disabled="disabled" <c:if test="${decideResult == 1 }"> checked="checked" </c:if> />匹配
						<input type="radio" name="decideResult" disabled="disabled" <c:if test="${decideResult == 0 }"> checked="checked" </c:if> />不匹配
					</td>
				</tr> --%>
				<tr>
					<td class="td_table_1">
