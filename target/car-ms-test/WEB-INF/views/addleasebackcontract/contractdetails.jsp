...
<div class="box">
    <div class="container">
        ...
        </div><div class="form-group newInput" style="width:33%;">
            <label>报价单：</label>
  ...
  <c:when test="${empty co.status}">   
    <div class="form-inline">
        <div class="form-group newInput" style="width:33%;">
            <label>状态：</label>门店部待审核
        </div>
    </div> 
  </c:when> 
  <c:when test="${co.status eq '1' or co.status eq '2'}">
  	<div class="form-inline">
  	<div class="form-group newInput" style="width:33%;">
        <label>是否零采：</label>${co.zeropur eq '1'?'是':co.zeropur eq '2'?'否':''}
    </div>  
    <div class="form-group newInput" style="width:33%;">
        <label>状态：</label>门店部审核${co.status eq '1'?'':'不'}通过
    </div>
    </div>
  </c:when>
  <c:when test="${co.status eq '3'}">
    ...
    <%-- <c:if test="${node eq '2'}"> --%>
    <div class="form-group newInput" style="width:33%;">
    <label>定金：</label>${co.downpay}
    </div>
    <div class="form-group newInput" style="width:33%;">
    <label>实到定金：</label>${co.realdownpay}
    </div>
    <%-- </c:if> --%>
    </div>
  </c:when>
  ...
  <c:otherwise>   
        ...
        <div class="du-btn">
            <a href="javascript:;" class="btn btn-warning" role="button" onclick="back()" style="padding: 6px 30px" id="backBtn">
                返回
            </a>
        </div>
    </div>
