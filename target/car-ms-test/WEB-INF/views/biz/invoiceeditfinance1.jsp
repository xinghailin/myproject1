			...
			<div class="container">
				...
				<form id="use_personal"><input type="hidden" id="ret" name="ret" value=""> <input type="hidden" id="conid" name="conid" value="${co.id }"><input type="hidden" id="id" name="id" value="${invoice.id }"><input type="hidden" id="currpage" name="currpage"
					...
					<div class="form-inline">
						...
						<div class="form-group newInput vali"><label><span style="color: red">*</span>动态监管：</label> <input type="text" class="form-control" name="fdynamicsupervise" value="${invoice.fdynamicsupervise}"></div>
						<div class="form-group newInput vali"><label><span style="color: red">*</span>履约险：</label> <input type="text" class="form-control" name="fappointinsurance" value="${invoice.fappointinsurance}"></div>
				...
				<div class="du-btn"><c:if test="${invoice.status eq 4}">
						<button class="btn btn-danger" role="button" id="submit" style="padding: 6px 38px" onclick="submit()">通过并开票</button>
					...
					</c:if> <a href="javascript:;" class="btn btn-warning" role="button" style="padding: 6px 38px" id="backBtn">返回</a></div>
		</div>
	</div>
</div>
</body>
...
<script>
	...
	valid:'glyphicon',invalid:'glyphicon glyphicon-remove',validating:'glyphicon glyphicon-refresh'},fields:{fsettlementcarprice:{message:'验证失败',validators:{notEmpty:{message:'不能为空'},stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},fpurchasetax:{message:'验证失败',validators:{notEmpty:{message:'不能为空'},stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},finsurance:{message:'验证失败',validators:{notEmpty:{message:'不能为空'},stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},fdynamicsupervise:{message:'验证失败',validators:{notEmpty:{message:'不能为空'},stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},fappointinsurance:{message:'验证失败',validators:{notEmpty:{message:'不能为空'},stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},fplateprice:{message:'验证失败',validators:{notEmpty:{message:'不能为空'},stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},fother1price:{message:'验证失败',validators:{stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},fother2price:{message:'验证失败',validators:{stringLength:{max:13,message:'金额超过最大限额'},regexp:{regexp:/((^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$))/,message:'金额格式填写错误'}}},fother1description:{message:'验证失败',validators:{stringLength:{max:255,message:'长度必须在小于255位字符'}}},fother2description:{message:'验证失败',validators:{stringLength:{max:255,message:'长度必须在小于255位字符'}}}}})
	function back(){
