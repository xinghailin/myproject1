		...
		<form name="editForm" id="editForm" method="POST"
			...
			<div class="tb_list">
				<ul class="tb_ul02">
					...
					<li>
						<div class="out">
							<div class="out-title">退税文件(clearance of goods files)</div>
							<div class="out-box">
								<div class="up-box">
									<input type="file" name="files" class="files">
									<div class="delete" style="width: 100px;" >删除(delete)</div>
								</div>
							</div>
							<div class="out-add" style="width: 200px;">继续添加(Continue to add)</div>
						</div>
						<div class="btnlist" style="text-align: center;">
							<input type="button" value="save" name="buttonSplit" class="btn63_01 margin_let10"
								onclick="mysubmit()">
							<a onclick="cancel()" id="btnReset" class="btn63_02 margin_let10">reset</a>
...
<script type="text/javascript">
	...
	$('.out-add')
			.click(
					function() {
						$(
								"<div class='up-box'> <input type='file' name='files'> <div class='delete'>删除</div> </div>")
								.appendTo($('.out-box'));
					});
	...
	function mysubmit() {
		$.each($('.up-box'), function() {
			if ($(this).find('.files').val() == "") {
				alert("Please upload the tax refund documents !");
				return false;
			} else {
				if (status) {
					status = false;
