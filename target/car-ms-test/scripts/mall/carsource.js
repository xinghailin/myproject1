define(function(require, exports, module) {
	...
	function formatstatus(status){
		...
		if(status==2)return "不展示";
		return "";
	}
	function operate(id,status){
		var nbsp = "&nbsp;",view,v,del,html; 
		view = $("<a href='javascript:void(0);' target='_self' name='viewBtn' class='btn63_03' id='"+id+"'/>").html('<span>查看</span>');
		...
		html=$('<span />').append(view).append(nbsp).
		append(v).append(nbsp).append(del).html();
	...
	function load(curPageNo, pageSize) {
		...
		$.ajax(queryUrl, {
			...
			if (response.result.length>0) {
				...
				//绑定列表按钮事件处理函数
				$("a[name='viewBtn']").on("click", viewClient);
...
//		$("#areaid option:first-child").prop("selected",true);
	}
...
var area=$('#areaid').val();
if(!area){alert('请选择区域	');return false;}
