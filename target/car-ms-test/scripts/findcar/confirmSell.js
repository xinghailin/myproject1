...
//		if($msg.val())$.dialog($msg.val(), function(){});
	...
	function _operate(id,state){
		var html;
		if (state == 3) {
			return '';
		}
			var edit = $("<a href='javascript:void(0);' target='_self' name='editBtn' class='btn63_03' id='"+id+"'/>").html('<span>核销</span>');
	...
	function editCar(){
		...
		$.dialog('确定核销？', function(){
		var del=$('#editUrl').val()+"?id="+theId+"&currpage="+$('#J_CurPageNo').text();
		$.ajax(del, {
			type : 'GET', 
			dataType : 'json'
		}).done(function(response) {
			if (response.status) {
				$.dialog('核销成功！', function(){
					load(1,PAGE_SIZE);
				});
			}else{
