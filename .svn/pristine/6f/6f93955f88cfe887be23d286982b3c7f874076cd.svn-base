define(function(require, exports, module) {
	...
	handlebars.registerHelper('_formatreason', _formatreason);
	...
	//格式化状态
	function _formatstatus(status2){
		if(2==status2){
			return '待审核';
		}else if(4==status2){
			return '审核不通过';
	...
	// 格式化原因
	function _formatreason(status2,reason){
		if(4==status2){
			return reason;
		}
	}
	function _addButton(status2,id){
		if(status2 == 2){
			var edit = $("<a href='javascript:void(0);' target='_self' name='viewBtn' class='btn63_03' id='"+id+"'/>").html('<span>审核</span>');
	...
	function viewCars(){
		location.href=$('#financecheckviewUrl').val()+"?id="+$(this).attr('id');
	}
});
