...
<input type="hidden" id="currpage" value="${currpage}"><input type="hidden" id="node" value="${node}">
<div class="main_right" id="viewList" style="margin-left:300px;width: 1550px;height: 100%;"><head>
    <meta charset="UTF-8">
    <title>合同<c:if test="${node == 1}">采购</c:if><c:if test="${node == 2}">法务</c:if><c:if test="${node == 3}">财务</c:if>查看(Contract Review)</title>
</head>
<body>
<div class="box">
    <h2 class="buy-title">合同<c:if test="${node == 1}">采购</c:if><c:if test="${node == 2}">法务</c:if><c:if test="${node == 3}">财务</c:if>查看(Contract Review)</h2>
    <div class="content">
        <form name="editForm" id="editForm" method="POST" action="<ufa:url namespace="car-ms" fieldName="jndContractsign"/>" enctype="multipart/form-data">
        ...
        <div class="rightbox" style="width: 100%;padding-left: 0px;border-left:0px;">
            <div class="right-title">
                ...
                <span>
                    ...
                    <i>(purchase file download)</i>
							...
							<p style="width: 80%;">
								<span>
									<i><label class="fc_red">*</label>合同文件下载:</i> <i>(contract file download)</i>
								</span>
								<a href="${contextPath}${con.texturl}" download="${con.texturlname}"
									style="text-decoration: underline; margin-left: 10px; color: #4d90fe;" title="点击下载">${con.texturlname}</a>
								<c:if test="${con.status eq '7'}">
									<c:if test="${empty con.stamp and node eq '0'}">
										<input id="text1url" name="text1url" type="file">
									</c:if>
									<c:if test="${con.stamp eq 1}">
										<c:if test="${node eq '0'}">
											<input id="text1url" name="text1url" type="file">
										</c:if>
										<a href="${contextPath}${con.text1url}" download="${con.texturl1name}"
											style="text-decoration: underline; margin-left: 10px; color: #4d90fe;" title="点击下载">${con.texturl1name}</a>
									</c:if>
								</c:if>
							</p>
						</div>
            <div class="right" style="padding-top: 5px;">
                ...
                <c:if test="${'1' eq con.selfbuy}"><div id="prediv">
					<div class="detail">
                    ...
                    <div>
                        ...
                        <input type="radio" name="paytype" value="1" <c:if test="${con.paytype eq 1}">checked="checked"</c:if> disabled="disabled">否(No)<input type="radio" name="paytype" value="2" style="margin-left: 33px;" <c:if test="${con.paytype eq 2}">checked="checked"</c:if> disabled>是(Yes)
                    </div>
                    ...
                    <div id="selfbuy">
                        ...
                        <input type="radio" name="selfbuy" value="1" disabled="disabled" <c:if test="${con.selfbuy eq 1}">checked="checked"</c:if>>自采(self-served)
                        <input type="radio" name="selfbuy" value="2" disabled="disabled" style="margin-left: 33px;" <c:if test="${con.selfbuy eq 2}">checked="checked"</c:if>>代采(agency-purchase)
                    </div></c:if>
                ...
                <div class="set">
                    ...
                    </span><span><textarea><c:if test="${empty con.status or con.status eq '10' or con.status eq '2' or con.status eq '5' or con.status eq '8'}">${con.rejectreason1}${con.rejectreason2}${con.rejectreason3}</c:if></textarea></span>
                </div><div class="set">
                    <span>
                        <i>不通过原因:</i>
                        <i>(No Pass Reason)</i>
                    </span><span><textarea><c:if test="${con.status eq '3' or con.status eq '6' or con.status eq '9'}">${con.rejectreason1}${con.rejectreason2}${con.rejectreason3}</c:if></textarea></span>
                </div></div></c:if>
                <div class="detail">
                <div>
                    <span>
                    ...
                    <c:when test="${con.status eq '10'}">已撤销(revoked)</c:when>
                    </c:choose></span>
                </div></div>
                <div class="detail">
    ...
    <div class="sell-button"><c:if test="${con.status eq '7' and node eq '0'}"><a href="javascript:void(0);" id="saveBtn">提交(submit)</a></c:if>
