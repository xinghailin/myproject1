define(function(require, exports, module) {
		...
		$.ajax(modelUrl, {
			...
			success : function(data) {
				...
				$.each(model, function(idx, val) {
					ddl.append($("<option value='" + val.model + "'>"
							+ val.model + "</option>"))
				})
			}
		})
		...
		$.ajax(versionUrl, {
			...
			success : function(data) {
				...
				$.each(version, function(idx, val) {
					ddl.append($("<option value='" + val.version + "'>"
							+ val.version + "</option>"))
				})
			}
		})
	...
	function saveFun() {
		...
		$form.validate({
			...
			/*
			 * , version : { required : "必填", maxlength:"输入过长" }, version : {
			 * required : "必填", maxlength:"输入过长" }
			 */
			/*
			 * ,leaveFactoryTime : { required : date }
			 */
			// ,
			// model : {
			// required : "必填",
	...
	/*
	 * $(function(){ $.ajax({
	 * //请求方式（post或get）默认为get。注意其他http请求方法，例如put和delete也可以使用，但仅部分浏览器支持。 type:
	 * 'POST', //String类型URL请求字符串。默认为当前页地址 url:
	 * "http://www.baidu.com/12345.action" , ////
	 * 发送到服务器的数据，它将被自动转为字符串类型。如果是GET请求，它将被附加到URL后面。jQuery会自动将对象数据转换为
	 * "name=codeplayer&age=18&uid=1&uid=2&uid=3" data: { name:"codeplayer",
	 * age:18, uid: [1, 2, 3] } , // 服务器返回的数据类型： //xml：返回XML文档，可用JQuery处理。
	 * //json：返回JSON数据 //text：返回纯文本字符串。 dataType: "json", //
	 * 同步请求，发送请求后浏览器将被锁定，只有等到该请求完成(无论成功或失败)后，用户才能操作，js代码才会继续执行 async: false,
	 * //指定请求完成(无论成功或失败)后需要执行的回调函数。该函数还有两个参数：
	 * //一个是jqXHR对象，一个是表示请求状态的字符串('success'、 'notmodified'、 'error'、 'timeout'、
	 * 'abort'或'parsererror')。 complete:function(textStatus, jqXHR){ }, //
	 * 假设返回的字符串数据为{ "name": "CodePlayer", age: 20 } //
	 * jQuery已帮我们将该JSON字符串转换为对应的JS对象，可以直接使用 success: function( data, textStatus,
	 * jqXHR ){ alert( data.name ); // CodePlayer } , // 请求失败时执行 // errorMsg
	 * 可能为： "Not Found"、"Internal Server Error"等 error: function(jqXHR,
	 * textStatus, errorMsg){ alert("请求失败：" + errorMsg); } }); });
	 */
});
