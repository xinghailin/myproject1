...
//		$("a[name='btnAddClientUrl']").click();
	...
	function operate(id){
		var nbsp = "&nbsp;";
		var edit = $("<a href='javascript:void(0);' target='_self' name='editBtn' class='btn63_03' id='"+id+"'/>").html('<span>编辑</span>');
...
//		var v=$("input[name='workType']:checked").val();
		...
		$.ajax(queryUrl, {
			...
			if (response.result.length>0) {
				...
				if(wt==2)source = $('#dataListTemplate1').html()
				...
				//绑定列表按钮事件处理函数
//				$("#btnAddClientUrl").on("click", addClient);
