...
//	require('jquery-css');
	...
	datepickerUtil.datesTriggerSimple('#leaveFactoryTime', null, null);
	var PAGE_SIZE = 10;
...
//	}
	...
	function load(curPageNo, pageSize) {
        dataList.show();
		noDataMsg.hide();var queryUrl;
		if(cid)
		queryUrl = $('#searchUrl').val() + "?id="+cid+"&start="+curPageNo+"&pageSize="+pageSize; // 查询数据url
		else 
			queryUrl = $('#searchUrl').val() + "?start="+curPageNo+"&pageSize="+pageSize;
		$.ajax(queryUrl, {
			type : 'POST',
			dataType : 'json',
...
//				window.opener.document.getElementById("vin").value=vin;
				var $doc=$(window.opener.document);$doc.find("#vin").attr("value",vin).val(vin);//$doc.find("#changeCarVin").attr("value",vin).val(vin);
//			}
//			else if(step==2){
