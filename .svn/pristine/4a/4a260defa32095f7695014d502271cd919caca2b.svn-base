	...
	<div class="main_right" id="viewList" style="margin-left: 300px; height: 100%;">
		<div class="box">
			<div class="container">
				...
				<h4 style="padding: 8px 0; font-size: 16px; margin-top: 15px" class="list-group-item-success">基本信息</h4> <input type="hidden" id="id" name="id" value="${co.id}">
				...
				<div class="form-inline"><div class="form-group newInput" style="width: 33%;"><label>车辆销售合同：</label> <a href="${contextPath}${co.salecontract}" download="${salecontractname}" style="text-decoration: underline; margin-left: 10px; color: #4d90fe;" title="点击下载">${salecontractname}</a></div>
				...
				<div class="form-inline">
					...
					<c:when test="${empty co.status}">
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>状态：</label>门店部待审核</div>
						</div>
					</c:when>
					<c:when test="${co.status eq '1'}">
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>是否零采：</label>${co.zeropur eq '1'?'是':co.zeropur eq '2'?'否':''}</div>
							<div class="form-group newInput" style="width: 33%;"><label>状态：</label>门店部审核通过</div>
						</div>
					</c:when>
					<c:when test="${co.status eq '2'}">
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>状态：</label>门店部审核不通过</div>
							<div class="form-group newInput" style="width: 33%;"><label>不通过原因：</label>${co.shopnopassreason}</div>
						</div>
					</c:when>
					<c:when test="${co.status eq '3'}">
						<c:if test="${co.finance eq '2'}">
							<div class="form-inline">
								<div class="form-group newInput" style="width: 33%;"><label>是否零采：</label>${co.zeropur eq '1'?'是':co.zeropur eq '2'?'否':''}</div>
								<div class="form-group newInput" style="width: 33%;"><label>门店部审核：</label>通过</div>
							</div>
							<div class="form-inline">
								<div class="form-group newInput" style="width: 33%;"><label>状态：</label>定金已到账</div> <%-- <c:if test="${node eq '2'}"> --%>
								<div class="form-group newInput" style="width: 33%;"><label>定金：</label>${co.downpay}</div>
								<div class="form-group newInput" style="width: 33%;"><label>实到定金：</label>${co.realdownpay}</div> <%-- </c:if> --%>
							</div>
						</c:if>
						<c:if test="${co.finance eq '1'}">
							<div class="form-inline">
								<div class="form-group newInput" style="width: 33%;"><label>是否零采：</label>${co.zeropur eq '1'?'是':co.zeropur eq '2'?'否':''}</div>
								<div class="form-group newInput" style="width: 33%;"><label>门店部审核：</label>通过</div>
							</div>
							...
							<div class="form-inline">
								<div class="form-group newInput" style="width: 33%;"><label>支付定金：</label>金融定金同意支付</div>
								<div class="form-group newInput" style="width: 33%;"><label>定金（首付）：</label>${co.downpay}</div>
								<div class="form-group newInput" style="width: 33%;"><label>状态：</label>定金已到账</div>
							</div>
							<div class="form-inline">
								<div class="form-group newInput" style="width: 33%;"><label>实到定金：</label>${co.realdownpay}</div>
							</div>
						</c:if>
					</c:when>
					<c:when test="${co.status eq '30'}">
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>是否零采：</label>${co.zeropur eq '1'?'是':co.zeropur eq '2'?'否':''}</div>
							<div class="form-group newInput" style="width: 33%;"><label>门店部审核：</label>通过</div>
						</div>
						...
						<div class="form-inline">
							...
							<div class="form-group newInput" style="width: 33%;"><label>状态：</label>已选车</div>
						</div>
					</c:when>
					<c:when test="${co.status eq '61'}">
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>是否零采：</label>${co.zeropur eq '1'?'是':co.zeropur eq '2'?'否':''}</div>
							<div class="form-group newInput" style="width: 33%;"><label>门店部审核：</label>通过</div>
						</div>
						...
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>状态：</label>金融定金同意支付</div>
							<div class="form-group newInput" style="width: 33%;"><label>定金（首付）：</label>${co.downpay}</div>
						</div>
					</c:when>
					<c:when test="${co.status eq '63'}">
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>是否零采：</label>${co.zeropur eq '1'?'是':co.zeropur eq '2'?'否':''}</div>
							<div class="form-group newInput" style="width: 33%;"><label>门店部审核：</label>通过</div>
						</div>
						...
						<div class="form-inline">
							<div class="form-group newInput" style="width: 33%;"><label>状态：</label>金融定金拒绝支付</div>
						</div>
					</c:when>
				</c:choose> <c:if test="${co.status2==1&&co.finance==1 }">
					<h4 style="padding: 8px 0; font-size: 16px; margin-top: 15px" class="list-group-item-success">金融验车信息</h4>
					<div class="form-inline">
						<div class="form-group newInput" style="width: 33%;"><label>车架号：</label>${co.vin}</div>
						<div class="form-group newInput" style="width: 33%;">` <label style="color: red;">验车结果：</label><label style="color: red;">通过</label>
					</div> <c:if test="${co.changevin!=null }">
							<!-- 验车通过更换车架号 -->
							<div class="form-group newInput" style="width: 33%;"><label style="color: red;">换车前的架号：</label><label style="color: red;">${co.changevin}</label></div>
						</c:if>
					...
					<div class="form-inline">
						<div class="form-group newInput" style="width: 33%;"><label>车架号：</label>${co.vin}</div>
						<div class="form-group newInput" style="width: 33%;">` <label style="color: red;">验车结果：</label><label style="color: red;">不通过</label>
					</div>
					</div>
				</c:if>
...
<script>
	seajs.use("${scriptBasePath}/biz/contractbaseview.js");
</script>
</html>