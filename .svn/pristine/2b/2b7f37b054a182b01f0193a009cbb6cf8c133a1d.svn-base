define(function(require, exports, module) {
	...
	function operate(status,id){
		...
		if(status==4){
			var move = $("<a href='javascript:void(0);' target='_self' name='moveBtn' class='btn63_03' id='"+id+"'/>").html('<span>发起物流</span>') ;
			var html=$('<span />').append(move).append(nbsp).append(look).html();
		}else if(status==5){
			var affirm = $("<a href='javascript:void(0);' target='_self' name='affirmBtn' class='btn63_03' id='"+id+"'/>").html('<span>物流确认到店</span>') ;
	...
	function load(curPageNo, pageSize) {
		...
		$.ajax(queryUrl, {
			...
			if (response.result.length>0) {
				...
				$("a[name='moveBtn']").on("click",moveBtn);
	...
	function lookBtn(){
		...
		location.href=$('#supplycartabledetailsUrl').val()+"?id="+theId+"&currpage="+$('#J_CurPageNo').text()+"&type=2";//查看
	};
	function moveBtn(){
		var theId=$(this).attr('id');
		location.href=$('#supplycartabledetailsUrl').val()+"?id="+theId+"&currpage="+$('#J_CurPageNo').text()+"&type=4";//发起物流
	};
	function affirmBtn(){
		var theId=$(this).attr('id');
		location.href=$('#supplycartabledetailsUrl').val()+"?id="+theId+"&currpage="+$('#J_CurPageNo').text()+"&type=5";//物流确认到店
	};
	function reset() {       
		form[0].reset();
	}
