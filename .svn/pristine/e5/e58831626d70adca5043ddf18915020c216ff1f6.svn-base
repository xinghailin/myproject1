...
<input type="hidden" id="backUrl" value="<ufa:url fieldName="mallcarsourceauditindex" namespace="car-ms"/>">
...
<body>
	<div class="main_right" id="viewList" style="margin-left: 300px; height: 100%;">
		<div class="box">
			<div class="container">
				<h3 class="title">审核车源</h3> <input type="hidden" name="unit" id="unit" value="${cs.unit }"><input type="hidden" id="id" name="id" value="${cs.id }">
				...
				<div class="form-inline">
					...
					<%-- <div class="form-group newInput"><label><span style="color: red;">*</span>外观颜色：</label> ${cs.outcolor}</div>
				...
				<div class="form-inline" style="width: 100%;">
					<div class="form-group newInput" style="width: 75%;"><label>配置：</label> <textarea name="config" id="config" class="form-control" placeholder="配置之间用^隔开" cols="84"
							rows="5" readonly="readonly">${cs.config}</textarea></div>
				...
				<h4 style="padding: 10px 0;" class="list-group-item-danger">审核</h4>
				<div class="form-inline">
					<div class="form-group newInput"><label><span style="color: red;">*</span>开证价：</label> <input type="text" class="form-control" id="issuingp" value="${cs.issuingprice}">${cs.unit}</div>
					<div class="form-group newInput"><label><span style="color: red;">*</span>预估报关费：</label> 根据实际价格收取</div>
					<div class="form-group newInput"><label><span style="color: red;">*</span>预估关税（开证价*汇率*税率）：</label><input type="hidden" id="exchange" value="${site.exchangerate }"><input type="hidden" id="tax" value="${cs.tax }"> <input type="text" class="form-control" id="estimate"
						value="${cs.duties}" readonly="readonly"></div>
					<div class="form-group newInput"><label><span style="color: red;">*</span>利息：</label> （车价+加装费-优惠）*汇率*8.5%/360*天数</div>
			</div>
				<div class="du-btn"><button class="btn btn-danger" role="button" id="submit" style="padding: 6px 38px" onclick="submit()">保存</button>
				<a href="javascript:;" class="btn btn-warning" role="button" style="padding: 6px 38px" id="backBtn">返回 </a></div>
		</div>
	</div>
</div>
</body>
...
<script src="${scriptBasePath}/mall/carsourceaudit.js?r=<%=new Date()%>"></script>
</html>