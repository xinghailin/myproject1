...
<div class="table_list" >
    <table cellpadding="0" cellspacing="0" class="tb_mian">
        ...
        <tr>
			...
			<th style="text-align:center">原始地址</th>
            ...
            <th style="text-align:center">一致性证书正面</th>
 			<th style="text-align:center">一致性证书反面</th>
...
<td>{{typeinsettleprice}}</td>
            <td>{{buyName}}</td>
            <td>{{_formatStatus status saleStatus mallCarStatus mallCarSkuCode}}</td>
        </tr>
        {{/result}}
        </tbody>
    </table>
...
<script type="text/javascript">
    seajs.use("${scriptBasePath}/assetRepository/assetlst.js?v=<%=new Date()%>");
</script>