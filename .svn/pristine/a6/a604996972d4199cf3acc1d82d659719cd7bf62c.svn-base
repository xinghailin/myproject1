...
<html lang="en">
	...
	<body>
		<form id="inputForm" action="${ctx }/flow/loan/applySave" method="post" target="mainFrame"  style="width:1524px;margin-left:350px">
			<input type="hidden" name="processId" value="${processId }" />
			<input type="hidden" name="orderId" value="${orderId }" />
			<input type="hidden" name="taskId" value="${taskId }" />
			<input type="hidden" name="borrow.id" value="${borrow.id }" />
			tAccountId:<input  name="tAccountId" value="" />
			...
			cellspacing="0" style="margin-top: 0px">
				<tr>
					...
					<td class="td_table_1"><span>申请借款日期：</span></td>
					<td class="td_table_2">
						&nbsp;${empty operateTime ? borrow.operateTime : operateTime } ${datetime}
					</td>
				</tr>
				<tr>
