...
//	handlebars.registerHelper('operate', operate);
	...
	function load(curPageNo, pageSize) {
        dataList.show();
		noDataMsg.hide();
		var queryUrl = $('#searchUrl').val() + "?start="+curPageNo+"&pageSize="+pageSize+"&forapay=1"; // 查询数据url
		$.ajax(queryUrl, {
			type : 'POST',
			dataType : 'json',
	...
	function havechoose(){
		...
		if(c.length>0){
			...
			window.location.href=$("#subbuyIndexUrl").val()+"?buyid="+theId+"&currpage="+$('#J_CurPageNo').text();
		}
		else{
